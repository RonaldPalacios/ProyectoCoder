...
const productController = { //creo un controlador para todas las vistas de productos
    ...
    getCategories: () => { // por categorias
      ...
      detail: (req, res) => { //detalle de los productos
        let data = productController.getProducts();
        let product = data.find(product => product.id == req.params.id);
        res.render("product/detail", { product: product });
      },
      listAll: (req, res) => { //todas los productos
        let data = productController.getProducts();
      ...
      listCategory: (req, res) => { //por categorias
        let data = productController.getProducts();
      ...
      crear: (req, res) => { //Crear Nuevo Producto
        let data = productController.getProducts();
        let nuevoProducto = {};
      ...
      edit: (req, res) => { //Para Editar
        let data = productController.getProducts();
        let product = data.find(product => product.id == req.params.id);
        res.render("adm-dashboard/modificarForm", { product: product });
      },
      update: (req, res) => { //Actualizacion del Mismo
        let data = productController.getProducts();
        let product = data.find(product => product.id == req.params.id);
      ...
      delete: (req, res) => { //Eliminacion del Producto
        let data = productController.getProducts();
